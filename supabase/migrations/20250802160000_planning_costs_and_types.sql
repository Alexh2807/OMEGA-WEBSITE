/*
  # Amélioration du système de planning avec types de soirées et coûts
  ...
  2. Modifications des tables existantes
    - `planning_locations`:
      - Ajout de `event_type_id` pour lier un lieu à un type de soirée par défaut.
    - `planning_providers`:
      - Ajout de `costs` (jsonb) pour stocker le coût d'un prestataire par type de soirée.
    - `planning_events`:
      - Suppression de `event_type_id`, car le type est maintenant hérité du lieu.

  3. Données initiales
    - Insertion des types de soirées demandés.

  4. Sécurité
    - Activation de RLS et ajout des politiques pour la nouvelle table.
*/
...
-- Étape 3: Modifier la table des lieux pour y lier un type de soirée
...
-- Étape 4: Modifier la table des prestataires pour ajouter les coûts
ALTER TABLE planning_providers
ADD COLUMN IF NOT EXISTS costs jsonb DEFAULT '{}'::jsonb;

-- Étape 5: (Optionnel) Nettoyer l'ancienne colonne si elle existait sur les événements
ALTER TABLE planning_events
DROP COLUMN IF EXISTS event_type_id;

-- Étape 6: Sécurité RLS pour la nouvelle table
