...
const AdminPlanningEditor: React.FC = () => {
  ...
  const loadEventTypes = async () => {
    ...
    if (error) toast.error('Erreur chargement types');
  ...
  const handleEventSubmit = async (e: React.FormEvent) => {
    ...
    let success = false;
    if (editingEvent) {
      success = await handleGenericSubmit(
  ...
  // ——— Export PDF : désactivation temporaire de dayMaxEvents pour stabilité des hauteurs ———
  const exportPlanningScreenshot = async (fileName: string) => {
    const api = calendarRef.current?.getApi();
    const prevDayMax = api?.getOption('dayMaxEvents');
    try {
      if (api) {
        api.setOption('dayMaxEvents', false);
        // Attendre la frame de rendu suivante pour que le layout soit stable
      ...
      return await exportElementAsPDF('planning-export', fileName);
    } finally {
      if (api && typeof prevDayMax !== 'undefined') {
        api.setOption('dayMaxEvents', prevDayMax);
  ...
  const getButtonClass = (monthValue: number) =>
    `px-3 py-1 text-sm rounded-md ${
      numberOfMonths === monthValue ? 'bg-blue-600 text-white' : 'bg-white/10 hover:bg-white/20 text-gray-300'
    }`;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3">
            <CalendarIcon className="text-blue-400" size={32} /> Planning Événementiel
          </h1>
        ...
        <div className="space-y-6">
          <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-md rounded-2xl p-6 border border-white/10">
            <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-3">
              <BarChart2 className="text-purple-400" /> Statistiques Détaillées (selon filtres)
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="bg-white/5 p-4 rounded-lg space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-gray-400 text-sm">Événements affichés</span>
                  <span className="text-xl font-bold text-white">{detailedStats.totalEvents}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-gray-400 text-sm">Coût Total Prestataires</span>
                  <span className="text-xl font-bold text-green-400">
                    {detailedStats.totalCost.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}
                  </span>
                </div>
              </div>
                    ...
                    detailedStats.eventTypeStats.map(([name, { count, cost }]) => (
                      <div key={name} className="flex justify-between items-center">
                        ...
                        <div className="text-right">
                          ...
                          <span className="text-green-400 text-xs ml-2">({cost.toFixed(2)}€)</span>
                        </div>
                      </div>
                    ))
                  ) : (
          ...
          <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-md rounded-2xl p-6 border border-white/10">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
              ...
              <div className="flex items-center gap-2">
                ...
                <select value={selectedEventType} onChange={e => setSelectedEventType(e.target.value)} className="dark-select w-full">
            ...
            <div className="flex justify-center md:justify-end items-center gap-4 mt-4">
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1 text-sm rounded-md transition-colors ${
                  isMultiSelectMode ? 'bg-blue-600 text-white' : 'bg-white/10 hover:bg-white/20 text-gray-300'
                }`}
              >
                <Layers size={16} /> Sélection Multiple
              </button>
          ...
          <div id="planning-export" className="calendar-container-dark relative">
            {multiSelectedDates.length > 0 && (
              <div className="sticky top-4 z-40 w-max mx-auto bg-gray-900/80 backdrop-blur-lg border border-white/20 rounded-xl shadow-2xl p-3 flex items-center gap-4 transition-all duration-300 animate-fade-in-up mb-4">
                <div className="flex items-center gap-2 text-white font-bold">
                  ...
                  <span>{multiSelectedDates.length} jour(s)</span>
                </div>
                <div className="h-8 w-px bg-white/20"></div>
                <div className="flex items-center gap-2">
                  <button onClick={handleBulkCreate} className="bg-blue-600 hover:bg-blue-500 text-white font-semibold px-4 py-2 rounded-lg text-sm">
                    <Plus size={16} /> Créer
                  </button>
                  <button onClick={handleBulkDelete} className="bg-red-600 hover:bg-red-500 text-white font-semibold px-4 py-2 rounded-lg text-sm">
            ...
            <style>{`
              ...
              .fc .fc-toolbar-title{color:#FFFFFF;font-weight:700}
              .fc .fc-button{
                background:var(--fc-button-bg-color);
                border:1px solid var(--fc-border-color);
                color:var(--fc-text-color);
                transition:background-color .3s;text-transform:capitalize
              }
              .fc .fc-button:hover{background:var(--fc-button-hover-bg-color)}
              .fc .fc-button-primary:not(:disabled).fc-button-active,
              .fc .fc-button-primary:not(:disabled):active{
                background:var(--fc-button-active-bg-color);
                border-color:var(--fc-button-active-bg-color)
              }
              .fc .fc-daygrid-day{border-color:var(--fc-border-color);transition:background-color .3s}
              .fc .fc-day-today{background-color:var(--fc-today-bg-color)!important}
              .fc .fc-daygrid-day-number{color:var(--fc-text-secondary-color);padding:.5em}
              .fc .fc-col-header-cell{background:rgba(255,255,255,0.05);color:var(--fc-text-secondary-color);border-color:var(--fc-border-color)}
              .fc .fc-daygrid-event{
                border-radius:4px;padding:2px 4px;margin-top:2px;font-size:.7rem;font-weight:500;box-shadow:0 2px 4px rgba(0,0,0,.2)
              }
              .fc .fc-daygrid-day.fc-day-future .fc-daygrid-day-number{color:var(--fc-text-color)}
              .fc-h-event .fc-event-main{padding:2px 4px}
            `}</style>
              ...
              eventContent={info => (
                <div className="p-1 overflow-hidden text-white text-[11px] h-full cursor-pointer">
                  <div className="flex items-center gap-1.5">
                    ...
                    ></div>
                    <b className="truncate block">{info.event.title}</b>
                  </div>
                  <p className="truncate italic opacity-80 pl-3.5">{info.event.extendedProps.eventType}</p>
                  <p className="truncate italic opacity-60 pl-3.5">{info.event.extendedProps.providers}</p>
                </div>
              )}
            />
          </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-sm">
                        {events.filter(e => e.provider_ids.includes(p.id)).length} événement(s)
                      </p>
                    </div>
                  </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {locations.map(l => (
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-4 h-4 rounded-full border-2 border-white/20 flex-shrink-0" style={{ backgroundColor: l.color }} />
                    <div>
                      <h4 className="text-white font-semibold">{l.name}</h4>
                      <p className="text-gray-400 text-sm">{l.event_type?.name || 'Type non défini'}</p>
                    </div>
                  </div>
                  <div className="flex gap-1">
                    <button
          ...
          <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-8 border border-white/10 max-w-2xl w-full">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-bold text-white">
                {editingEvent ? `Modifier: ${new Date(editingEvent.event_date + 'T00:00:00').toLocaleDateString('fr-FR')}` : 'Nouvel événement'}
              </h3>
              <button onClick={resetEventForm} className="text-gray-400 hover:text-white text-2xl">
                ×
              </button>
            </div>
            ...
            {!editingEvent && multiSelectedDates.length > 0 && (
              <p className="text-center text-blue-300 mb-4 bg-blue-500/10 py-2 rounded-lg">
                Création sur <b>{multiSelectedDates.length} dates</b>
              </p>
            )}
            <form onSubmit={handleEventSubmit} className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Prestataires *</label>
              ...
              <div className="flex gap-4">
                <button
                  type="submit"
                  className="flex-1 bg-gradient-to-r from-blue-500 to-blue-600 text-white py-3 rounded-lg font-semibold flex items-center justify-center gap-2"
                >
                  <Save size={16} />
                ...
                {editingEvent && (
                  <button
                    ...
                    className="flex-1 bg-gradient-to-r from-red-500 to-red-600 text-white py-3 rounded-lg font-semibold flex items-center justify-center gap-2"
                  >
                    <Trash2 size={16} />
                    Supprimer
                  </button>
      ...
      {showProviderModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-8 border border-white/10 max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-bold text-white">{editingProvider ? 'Modifier prestataire' : 'Nouveau prestataire'}</h3>
              <button onClick={resetProviderForm} className="text-gray-400 hover:text-white text-2xl">
                ×
              </button>
            </div>
            <form onSubmit={handleProviderSubmit} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:border-blue-400"
                  placeholder="Ex: DJ Martin"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Coûts par type de soirée (€)</label>
                <div className="space-y-3 bg-white/5 p-3 rounded-lg border border-white/20">
                  {eventTypes.map(et => (
                    <div key={et.id} className="grid grid-cols-3 items-center gap-2">
                      ...
                      <input
                        ...
                        onChange={e => setProviderForm(f => ({ ...f, costs: { ...f.costs, [et.id]: e.target.value } }))}
                        className="col-span-1 w-full bg-white/10 border border-white/20 rounded-md px-2 py-1 text-white placeholder-gray-400 focus:border-blue-400 text-right"
                        placeholder="0.00"
                      />
                    </div>
                  ))}
              ...
              <div className="flex gap-4">
                <button type="submit" className="flex-1 bg-gradient-to-r from-blue-500 to-blue-600 text-white py-3 rounded-lg font-semibold">
                  {editingProvider ? 'Mettre à jour' : 'Ajouter'}
                </button>
                <button type="button" onClick={resetProviderForm} className="px-6 border-2 border-white/30 text-white rounded-lg font-semibold">
                  Annuler
                </button>
              </div>
            </form>
      ...
      {showLocationModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl p-8 border border-white/10 max-w-md w-full">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-bold text-white">{editingLocation ? 'Modifier le lieu' : 'Nouveau lieu'}</h3>
              <button onClick={resetLocationForm} className="text-gray-400 hover:text-white text-2xl">
                ×
              </button>
            </div>
            <form onSubmit={handleLocationSubmit} className="space-y-6">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/20 rounded-lg px-4 py-3 text-white focus:border-blue-400"
                  placeholder="Ex: Salle des fêtes"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Type de soirée par défaut *</label>
                <select
                  required
                  value={locationForm.event_type_id}
                  onChange={e => setLocationForm({ ...locationForm, event_type_id: e.target.value })}
                  className="dark-select w-full"
                >
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <input
                    ...
                    value={locationForm.color}
                    onChange={e => setLocationForm({ ...locationForm, color: e.target.value })}
                    className="flex-1 bg-white/5 border border-white/20 rounded-lg px-4 py-3 text-white focus:border-green-400"
                    placeholder="#3B82F6"
                  />
                </div>
              </div>
              <div className="flex gap-4">
                <button type="submit" className="flex-1 bg-gradient-to-r from-green-500 to-green-600 text-white py-3 rounded-lg font-semibold">
                  {editingLocation ? 'Mettre à jour' : 'Ajouter'}
                </button>
                <button type="button" onClick={resetLocationForm} className="px-6 border-2 border-white/30 text-white rounded-lg font-semibold">
                  Annuler
                </button>
              </div>
            </form>
