...
const AdminPlanningEditor: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const channels = [
      supabase.channel('planning_events_updates').on('postgres_changes', { event: '*', schema: 'public', table: 'planning_events' }, reloadVisibleEvents).subscribe(),
      supabase.channel('planning_locations_updates').on('postgres_changes', { event: '*', schema: 'public', table: 'planning_locations' }, () => { loadLocations(); reloadVisibleEvents(); }).subscribe(),
      supabase.channel('planning_providers_updates').on('postgres_changes', { event: '*', schema: 'public', table: 'planning_providers' }, () => { loadProviders(); reloadVisibleEvents(); }).subscribe()
    ];
  ...
  const handleBulkCreate = () => { if (multiSelectedDates.length === 0) return; setEditingEvent(null); setSelectionInfo(null); setEventForm({ location_id: locations[0]?.id || '', provider_ids: [] }); setShowEventModal(true); };
  const handleBulkDelete = async () => {
    ...
    if (eventsToDelete.length === 0) { toast.success("Aucun événement à supprimer."); setMultiSelectedDates([]); return; }
    const { error } = await supabase.from('planning_events').delete().in('id', eventsToDelete.map(e => e.id));
    if (error) toast.error("Erreur de suppression en masse.");
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="flex items-center justify-between"><h1 className="text-3xl font-bold text-white mb-2 flex items-center gap-3"><CalendarIcon className="text-blue-400" size={32}/> Planning Événementiel</h1><button onClick={exportPlanningPDF} className="bg-gradient-to-r from-green-500 to-green-600 text-white px-4 py-2 rounded-lg font-semibold hover:shadow-lg hover:shadow-green-500/25 transition-all duration-300 flex items-center gap-2"><Download size={16}/> Export PDF</button></div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4"><div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-md rounded-lg p-4 border border-white/10"><div className="text-2xl font-bold text-white">{filteredEventsForCalendar.length}</div><div className="text-gray-400 text-sm">Événements Visibles</div></div><div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-md rounded-lg p-4 border border-white/10"><div className="text-2xl font-bold text-blue-400">{providers.length}</div><div className="text-gray-400 text-sm">Prestataires</div></div><div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-md rounded-lg p-4 border border-white/10"><div className="text-2xl font-bold text-green-400">{locations.length}</div><div className="text-gray-400 text-sm">Lieux</div></div><div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-md rounded-lg p-4 border border-white/10"><div className="text-2xl font-bold text-yellow-400">{events.filter(e => new Date(e.event_date) >= new Date()).length}</div><div className="text-gray-400 text-sm">À venir</div></div></div>
      ...
      {/* Barre d'actions contextuelle pour la multi-sélection */}
