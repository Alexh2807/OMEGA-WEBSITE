...
interface Provider {
  id: string;
  name: string;
  costs: { [key: string]: number }; // Pour stocker les coûts par type d'événement
  created_at: string;
}
...
interface Location {
  ...
  event_type?: EventType; // La jointure peut être nulle
  created_at: string;
}
...
interface EventItem {
  ...
  provider_ids: string[];
  location_id: string;
  location: Location | null; // La jointure inclut maintenant le type d'événement
  created_at: string;
}
...
const AdminPlanningEditor: React.FC = () => {
  ...
  const loadLocations = async () => {
    const { data, error } = await supabase.from('planning_locations').select('*, event_type:planning_event_types(*)').order('name');
  ...
  const handleGenericSubmit = async (action: any, successMessage: string): Promise<boolean> => {
    ...
    if (error) { toast.error("Une erreur est survenue."); return false; }
  ...
  const handleLocationSubmit = async (e: React.FormEvent) => {
    ...
    const action = editingLocation
      ? supabase.from('planning_locations').update({ ...locationForm, event_type_id: locationForm.event_type_id || null }).eq('id', editingLocation.id)
    ...
    return [
      ...filteredEvents.map(e => ({ 
        ...
        extendedProps: { 
            ...
            eventType: e.location?.event_type?.name || 'Non défini'
        } 
      })), 
      ...backgroundSelection
    ];
      ...
      {activeTab === 'calendar' && (
        <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-md rounded-lg p-4 border border-white/10"><div className="text-2xl font-bold text-purple-400">{eventTypeStats.length}</div><div className="text-gray-400 text-sm">Types de soirées</div></div>
                <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-md rounded-lg p-4 border border-white/10"><div className="text-2xl font-bold text-green-400">{totalCost.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' })}</div><div className="text-gray-400 text-sm">Coût total prestataires</div></div>
                <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/50 backdrop-blur-md rounded-lg p-4 border border-white/10"><div className="text-2xl font-bold text-yellow-400">{events.filter(e => e.event_date >= toYYYYMMDD(new Date())).length}</div><div className="text-gray-400 text-sm">Événements à venir</div></div>
            </div>
