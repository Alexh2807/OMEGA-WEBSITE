...
10. **NOUVEAU — Livraison professionnelle (v2 le 11/07 au soir)** : chaque produit a un **gabarit** (fiche produit admin) + un **poids unitaire (kg)**. *Colis* = barème au **poids total** par tranches réglables (défaut : ≤1 kg 7,99 € · ≤2 kg 15,90 € · ≤5 kg 19,90 € · ≤10 kg 29,90 € · ≤30 kg 49,90 € · au-delà 79,90 €). *Palette/encombrant* (machine, bidon volumineux même léger) = par unité selon la **zone calculée automatiquement** : France 0–200 km (90 €) / 200–500 km (140 €) / entière (250 €) via distance routière estimée depuis le dépôt de Montblanc (haversine département × 1,25, sans API externe, Corse = France entière) ; **Europe** (280 €) et **Express Europe** (450 €, au choix du client) selon le pays ; DOM-TOM / hors Europe = **devis** (bouton dédié dans le panier). Colis sans surcoût avec une palette ; surcoût colis Europe 12 €. Tout est réglable dans **Admin → Paramètres → Livraison** (tranches dynamiques, prix par zone, seuils km, poids par défaut, délai). Stripe encaisse produits + livraison ; `orders.shipping_cost`/`shipping_method` enregistrés. 26 tests unitaires (`src/utils/__tests__/shipping.test.ts`). Migration `20260711150000_product_weight.sql` (appliquée en prod le 11/07).
